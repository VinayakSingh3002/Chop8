  ...
  return (
    <div className={styles.services}>
      <div className={styles.searchBox}>
        <input
          type="text"
          placeholder="Search by chef name or specialisation..."
          value={search}
          onChange={e => setSearch(e.target.value)}
        />
      </div>

      {/* Active specialisation filter badge */}
      {search && (
                ...
                <div key={i} style={{ display:"flex", flexDirection:"column", alignItems:"center", width:"220px" }}>
                  <div style={{ position:"relative", width:"100%" }}>
                    ...
                    <Card
                      ...
                      onBook={() => {
                        if (!chef.available) {
                          alert(`${chef.name} is currently unavailable for booking.`);
                          return;
                        }
                  ...
                  {selectedChef?.id === chef.id && (
                    <BookingForm
                      ...
                      onBooked={(booking) => {
                        alert(`Booking confirmed! Token: ${booking.tokenId}`);
                        setSelectedChef(null);
                        setMyBookings(prev => [...prev, booking]);
                        if (booking.date === today) setBusyMap(prev => ({ ...prev, [chef.id]: true }));
                      }}
                    />
                  )}
                </div>
